<script>
	...
	import LogoBackground from './LogoBackgroud.svelte';
...
>
	...
	<div class="relative">
		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.repeatablezone as item, index}
				{#if item.icon}
					{#if index === Math.floor(slice.primary.repeatablezone.length / 2)}
						<StylizedLogoMark />
					...
					{#if index !== slice.items.length - 1}
						<div
							class={clsx(
								'signal-line',
								index >= Math.floor(slice.primary.repeatablezone.length / 2) ? 'rotate-180' : 'rotate-0'
							)}
						></div>
					{/if}
				{/if}
